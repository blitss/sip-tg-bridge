...
func TestIntegrationDialogServerPlayback(t *testing.T) {
	...
	dialog := &DialogServerSession{
		DialogMedia: DialogMedia{
			mediaSession:    &media.MediaSession{Codecs: []media.Codec{codec}},
			RTPPacketWriter: media.NewRTPPacketWriter(rtpBuf, codec),
		},
	}
	...
	assert.Greater(t, diffTS2, diffTS+5*codec.SampleTimestamp())
}
