...
func (dec *PCMDecoder) Init(codec media.Codec) error {
	...
	switch {
	case codec.PayloadType == FORMAT_TYPE_ULAW:
		dec.DecoderTo = DecodeUlawTo
	case codec.PayloadType == FORMAT_TYPE_ALAW:
		dec.DecoderTo = DecodeAlawTo
	// Opus uses dynamic payload types, so don't key off PT. Use encoding name instead.
	case strings.EqualFold(codec.Name, "opus"):
		opusDec := OpusDecoder{}
