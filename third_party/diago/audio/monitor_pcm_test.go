...
func TestMonitorPCMReaderWriter(t *testing.T) {
	codecR := media.CodecAudioAlaw(media.DefaultSampleDur())

	audioAlawBuf := make([]byte, 4*160)
	_, err := EncodeAlawTo(audioAlawBuf, bytes.Repeat([]byte("0123456789"), codecR.Samples16()*4/10))
...
func TestMonitorPCMStereo(t *testing.T) {
	codec := media.CodecAudioAlaw(media.DefaultSampleDur())
	audioAlawBuf := make([]byte, 4*160)
	_, err := EncodeAlawTo(audioAlawBuf, bytes.Repeat([]byte("0123456789"), codec.Samples16()*4/10))
	require.NoError(t, err)
	...
	t.Run("SmallData", func(t *testing.T) {
		...
		recording := bytes.NewBuffer([]byte{})
		err = mon.Init(recording, codec, bytes.NewBuffer(audioAlawBuf), bytes.NewBuffer([]byte{}))
		require.NoError(t, err)
	...
	t.Run("BigData", func(t *testing.T) {
		...
		mon := &MonitorPCMStereo{}
		recording := bytes.NewBuffer([]byte{})
		err = mon.Init(recording, codec, bytes.NewBuffer(audioAlawBufBig), bytes.NewBuffer([]byte{}))
		require.NoError(t, err)
