...
func TestMediaSessionUpdateCodec(t *testing.T) {
	ulaw := CodecAudioUlaw(DefaultSampleDur())
	alaw := CodecAudioAlaw(DefaultSampleDur())
	dtmf := CodecTelephoneEvent8000(DefaultSampleDur())
	...
	m := newM()
	m.updateRemoteCodecs([]Codec{alaw, ulaw})
	// We prefer PCMU over PCMA even if the offer lists PCMA first.
	assert.Equal(t, []Codec{ulaw, alaw}, m.filterCodecs)

	m = newM()
	m.updateRemoteCodecs([]Codec{alaw})
	assert.Equal(t, []Codec{alaw}, m.filterCodecs)
...
a=sendrecv`
	...
	m := MediaSession{
		Codecs: []Codec{
			CodecAudioAlaw(DefaultSampleDur()),
			CodecAudioUlaw(DefaultSampleDur()),
			CodecTelephoneEvent8000(DefaultSampleDur()),
		},
	...
	assert.Equal(t, CodecAudioUlaw(DefaultSampleDur()), m.filterCodecs[0])
	assert.Equal(t, CodecAudioAlaw(DefaultSampleDur()), m.filterCodecs[1])
	assert.Equal(t, CodecTelephoneEvent8000(DefaultSampleDur()), m.filterCodecs[2])

	lsdp := m.LocalSDP()
	lsd := sdp.SessionDescription{}
	sdp.Unmarshal(lsdp, &lsd)

	// Check that local preference order is used in answer.
	assert.Equal(t, "audio 1234 RTP/AVP 0 8 101", lsd.Value("m"))
...
a=sendrecv`
		...
		assert.Equal(t, CodecAudioUlaw(DefaultSampleDur()), m.filterCodecs[0])
		assert.Equal(t, CodecTelephoneEvent8000(DefaultSampleDur()), m.filterCodecs[1])
	}
}
...
func TestMediaSRTP(t *testing.T) {
	m1 := MediaSession{
		Laddr:     net.UDPAddr{IP: net.IPv4(127, 0, 0, 1)},
		Codecs:    []Codec{CodecAudioAlaw(DefaultSampleDur())},
		SecureRTP: 1,
		SRTPAlg:   SRTPAes128CmHmacSha1_80,
		Mode:      sdp.ModeSendrecv,
	}
