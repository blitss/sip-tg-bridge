...
func (s *RTPSession) WriteRTP(pkt *rtp.Packet) error {
	...
	if writeStats.SSRC != pkt.SSRC {
		codec, err := func() (Codec, error) {
			// IMPORTANT: dynamic payload types (96-127) are negotiated via SDP and
			// the negotiated PT may differ from the local offer PT.
			//
			// When we write RTP, the packet PT must match the negotiated mapping,
			// so prefer searching negotiated (common) codecs first.
			for _, codecs := range [][]Codec{s.Sess.filterCodecs, s.Sess.Codecs} {
				for _, c := range codecs {
