...
func (s *MediaSession) updateRemoteCodecs(codecs []Codec) int {
	...
	for _, rc := range codecs {
		for _, c := range s.Codecs {
			// Match by SDP identity rather than struct equality so that:
			// - dynamic payload types (e.g. opus/48000/2 as PT=95) negotiate correctly
			// - negotiated payload types come from the offer/answer (rc.PayloadType)
			//
			// NOTE: We intentionally do NOT require payload type equality here.
			if strings.EqualFold(c.SDPName(), rc.SDPName()) {
				filter = append(filter, rc)
				break
			}
		}
	}
	// Prefer our best codec even if the offer listed it later.
	// This influences answer ordering (and therefore what many UAs will actually use).
	SortCodecsByPreference(filter)
...
func generateSDPForAudio(rtpProfile string, originIP net.IP, connectionIP net.IP, rtpPort int, mode string, codecs []Codec, sdes sdesInline) []byte {
	...
	for i, f := range codecs {
		formatsMap = append(formatsMap, fmt.Sprintf("a=rtpmap:%d %s", f.PayloadType, f.SDPName()))
		if f.IsDTMF() {
