...
func TestIntegrationDiagoCallWithCustomCodecs(t *testing.T) {
...

	{
		...
		dg := NewDiago(ua,
			...
			WithMediaConfig(
				MediaConfig{
					Codecs: []media.Codec{l16Codec, media.CodecAudioAlaw(media.DefaultSampleDur())},
				},
			))
	...
	defer cancel()
	{
		...
		dg := NewDiago(ua,
			...
			WithMediaConfig(
				MediaConfig{
					Codecs: []media.Codec{
						media.CodecAudioUlaw(media.DefaultSampleDur()),
						media.CodecAudioAlaw(media.DefaultSampleDur()),
					},
				},
			))
