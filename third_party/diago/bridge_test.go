	...
	incoming := &DialogServerSession{
		DialogMedia: DialogMedia{
			mediaSession: &media.MediaSession{
				Codecs: []media.Codec{codec},
			},
			audioReader:     bytes.NewBuffer(make([]byte, 9999)),
			audioWriter:     bytes.NewBuffer(make([]byte, 0)),
			RTPPacketReader: media.NewRTPPacketReader(nil, codec),
			RTPPacketWriter: media.NewRTPPacketWriter(nil, codec),
		},
	}
...
func TestBridgeNoTranscodingAllowed(t *testing.T) {
	b := NewBridge()
	// b.waitDialogsNum = 99 // Do not start proxy
	inCodec := media.CodecAudioAlaw(media.DefaultSampleDur())
	outCodec := media.CodecAudioUlaw(media.DefaultSampleDur())

	incoming := &DialogServerSession{
		DialogMedia: DialogMedia{
			mediaSession: &media.MediaSession{
				Codecs: []media.Codec{inCodec},
			},
