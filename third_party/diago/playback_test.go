...
func TestIntegrationStreamWAV(t *testing.T) {
	...
	codec := media.CodecAudioUlaw(media.DefaultSampleDur())
...
func TestIntegrationPlaybackFile(t *testing.T) {
	...
	dialog := &DialogServerSession{
		DialogMedia: DialogMedia{
			mediaSession: &media.MediaSession{Codecs: []media.Codec{media.CodecAudioUlaw(media.DefaultSampleDur())}},
			// audioReader:  bytes.NewBuffer(make([]byte, 9999)),
			audioWriter:     w,
			RTPPacketWriter: media.NewRTPPacketWriter(nil, media.CodecAudioUlaw(media.DefaultSampleDur())),
		},
	}
